...
useSeoMeta({
  ...
  ogTitle: `สร้างโปรเจกต์ - ${runtimeConfig.public.SiteName}`,
});
...
const editMember = (id) => {
  editMemberModal.value = {
    id: id,
    name: memberList.value[id].name,
    profileImage: memberList.value[id].image,
    role: memberList.value[id].role,
  };
  ...
  <div class="row gy-4 mb-5">
    ...
    <div class="col-md-8">
      ...
      <div class="row align-items-center" v-for="(member, index) in memberList">
        ...
        <div class="col-auto p-1">
          <button class="btn btn-warning btn-sm" @click="editMember(index)">
            แก้ไข
          </button>
        </div>
        <div class="col-auto p-1">
          <button class="btn btn-danger btn-sm" @click="onMemberRemove(index)">
            ลบ
          </button>
        </div>
      </div>
      <button class="btn btn-success mt-3" @click="addMember">
        สร้างสมาชิกเพิ่ม
      </button>
    </div>
    <hr />
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5 sarabun-extrabold" id="editMemberModal">
            แก้ไขข้อมูลlสมาชิก
          </h1>
          <button
            type="button"
